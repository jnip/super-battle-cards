...
public class FirebaseInterface extends DatabaseInterface {
  private int indexLength = 10;
  ...
  public int getKills(String raw) {
    // {"move_INDEX_KILLS"...
  ...
  public HashMap parseObject(String data) {
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      System.out.println("FirebaseInterface failed to process: "+Arrays.toString(field));
    }
    return map;
  }
...
*/
  public String[][][] parseBoard(String data) {
    if (data.matches("\\[\\[.*\\]\\]")) {
      ...
      for (int i = 0; i < arrays.length; i++) {
        ...
        for (int j = 0; j < objects.length; j++) {
          ...
          nameData[0][i][j] = this.getObjectName(objects[j]);
  ...
  private String getObjectName(String data) {
    String toMatch = "\"name\":";
